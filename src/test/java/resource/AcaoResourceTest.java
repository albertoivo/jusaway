...
public class AcaoResourceTest {
	...
	public void init() {
		try (Connection con = new ConnectionPool().getConnection()) {
			acao = new Acao(NOME_1, DESCRICAO_1);
			dao = new AcaoDao(con);
			dao.persist(acao);
	...
	public void finish() {
		try (Connection con = new ConnectionPool().getConnection()) {
			try (PreparedStatement stmt = con.prepareStatement("truncate table acao")) {
				stmt.execute();
			}
		} catch (SQLException e) {
			fail(e.getMessage());
	...
	public void trazTodos() {
		String result = target.path("acoes").request().get(String.class);
	...
	public void trazUm() {
		...
		Acao novaAcao = jsonPath.getObject("", Acao.class);
		assertEquals(novaAcao.getNome(), NOME_1);
	}

	@Test
	public void trazOutro() {
		get("/acoes/" + NOME_1).then().body("nome", equalTo(NOME_1));
	}
	
	@Test
	public void trazNenhum() {
		get("/acoes/" + ID_QUE_NAO_EXISTE).equals(null);
	}
	...
	public void adicionaUm() {
		...
		.body(new Acao(NOME_2, DESCRICAO_2))
	...
	public void atualizaUm() throws SQLException {
		URI uri = URI.create(LOCALHOST + "/acoes/" + NOME_1);
		Acao novaAcao = new Acao(NOME_1, NOVA_DESCRICAO);
		given()
		.contentType(ContentType.JSON)
		.body(novaAcao)
		.expect().statusCode(Response.Status.OK.getStatusCode())
	...
	public void atualizaNaoExistente() throws SQLException {
		URI uri = URI.create(LOCALHOST + "/acoes/" + ID_QUE_NAO_EXISTE);
		Acao novaAcao = new Acao(NOME_1, NOVA_DESCRICAO);
		given()
		.contentType(ContentType.JSON)
		.body(novaAcao)
