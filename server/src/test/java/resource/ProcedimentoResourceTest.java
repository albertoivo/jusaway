...
public class ProcedimentoResourceTest {
	...
	public void init() {
		try (Connection con = new ConnectionPool().getConnection()) {
			procedimento = new Procedimento(NOME_1, DESCRICAO_1);
	...
	public void trazTodos() {
		String result = target.path("procedimentos").request().get(String.class);
	...
	public void trazUm() {
		JsonPath jsonPath = given()
			...
			.get("/procedimentos/" + NOME_1)
			.andReturn().jsonPath();

		Procedimento novaProcedimento = jsonPath.getObject("", Procedimento.class);
		assertEquals(novaProcedimento.getNome(), NOME_1);
	}

	@Test
	public void trazOutro() {
		get("/procedimentos/" + NOME_1).then().body("nome", equalTo(NOME_1));
	}
	
	@Test
	public void trazNenhum() {
		get("/procedimentos/" + ID_QUE_NAO_EXISTE).equals(null);
	}
	...
	public void adicionaUm() {
		...
		.body(new Procedimento(NOME_2, DESCRICAO_2))
	...
	public void atualizaUm() throws SQLException {
		URI uri = URI.create(LOCALHOST + "/procedimentos/" + NOME_1);
		Procedimento novaProcedimento = new Procedimento(NOME_1, NOVA_DESCRICAO);
		given()
		.contentType(ContentType.JSON)
		.body(novaProcedimento)
		.expect().statusCode(Response.Status.OK.getStatusCode())
		.when()
		.put(uri);
	}
	...
	public void atualizaNaoExistente() throws SQLException {
		URI uri = URI.create(LOCALHOST + "/procedimentos/" + ID_QUE_NAO_EXISTE);
		Procedimento novaProcedimento = new Procedimento(NOME_1, NOVA_DESCRICAO);
		given()
		.contentType(ContentType.JSON)
		.body(novaProcedimento)
